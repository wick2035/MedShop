...
export default function HospitalDetailPage() {
  ...
  const handleStatusChange = async (status: HospitalStatus) => {
    ...
    try {
      ...
      toast.success(`Hospital status updated to ${HOSPITAL_STATUS_LABELS[status]}`);
      fetchHospital();
    } catch (err) {
    ...
    <PageContainer>
      ...
      >
        ...
        <div className="mt-4 flex items-center gap-2 text-xs text-gray-400">
          <span>Created: {formatDateTime(hospital.createdAt)}</span>
          <span>|</span>
          <span>Updated: {formatDateTime(hospital.updatedAt)}</span>
        </div>

        {/* Status change actions */}
        <div className="mt-6 border-t border-ivory-200 pt-4">
          <p className="mb-3 text-sm font-medium text-gray-600">Change Status</p>
